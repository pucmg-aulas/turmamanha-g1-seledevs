        ...
        for (int i = 0; i < Util.alfabeto().size(); i++) {
            for (int j = 0; j < this.vagasPorFileira; j++) {
                ...
                if (nIdoso != 0) {
                    ...
                    VagaDao.addVaga(new Idoso(this, Util.alfabeto().get(i).toString() + j, false));
                    nIdoso--;
                } else if (nPCD != 0) {
                    this.vagas.add(new PCD(this, Util.alfabeto().get(i).toString() + j, false));
                    VagaDao.addVaga(new PCD(this, Util.alfabeto().get(i).toString() + j, false));
                    nPCD--;
                } else if (nVIP != 0) {
                    this.vagas.add(new VIP(this, Util.alfabeto().get(i).toString() + j, false));
                    VagaDao.addVaga(new VIP(this, Util.alfabeto().get(i).toString() + j, false));
                    nVIP--;
                } else {
                    this.vagas.add(new Comum(this, Util.alfabeto().get(i).toString() + j, false));
                    VagaDao.addVaga(new Comum(this, Util.alfabeto().get(i).toString() + j, false));
                }
