...
public class Parque implements Serializable {
    ...
    private boolean validarNumeroDeVagas() {
        ...
        return maximo >= this.numeroVagas;
    }
    ...
    public void montarVagas() {
        ...
        for (int i = 0; i < Util.alfabeto().size(); i++) {
            for (int j = 0; j < this.vagasPorFileira; j++) {
                ...
                if (nIdoso > 0) {
                    IdosoModel vaga = new IdosoModel(this, Util.alfabeto().get(i).toString() + j, false);
                    this.vagas.add(vaga);
                    VagaDao.gravar(vaga);
                    nIdoso--;
                } else if (nPCD > 0) {
                    PcdModel vaga = new PcdModel(this, Util.alfabeto().get(i).toString() + j, false);
                    this.vagas.add(vaga);
                    VagaDao.gravar(vaga);
                    nPCD--;
                } else if (nVIP > 0) {
                    VipModel vaga = new VipModel(this, Util.alfabeto().get(i).toString() + j, false);
                    this.vagas.add(vaga);
                    VagaDao.gravar(vaga);
                    nVIP--;
                } else {
                    ComumModel vaga = new ComumModel(this, Util.alfabeto().get(i).toString() + j, false);
                    this.vagas.add(vaga);
                    VagaDao.gravar(vaga);
                }
